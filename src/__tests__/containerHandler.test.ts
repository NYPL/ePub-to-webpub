...
describe('Container Handler', () => {
  it('should return JSON manifest on success', async () => {
    ...
    const testRequest = {
		...
		expect(mockResp.writeHead).toHaveBeenCalledWith(
			200,
			{
  ...
  it('should return static message on root path', async () => {
    ...
    const testRequest = {
		...
		expect(mockResp.end).toHaveBeenLastCalledWith("{\"service\":\"epub-to-webpub\"}", 'utf-8');
	})
  ...
  it('should return 404 for other paths', async () => {
    ...
    const testRequest = {
		...
		expect(mockResp.end).toHaveBeenLastCalledWith(
			"{\"title\":\"Unknown path\",\"detail\":\"testServer/testing\"}", 'utf-8'
		);
	})
